class Api::UsersController < Api::ApplicationController
  ...
  before_action :authenticate_user!, only: %i[show update user_spots_count_each_mood show_current_user likes already_liked?]
  before_action :correct_user, only: %i[show update user_spots_count_each_mood likes already_liked?]
  ...
  def update_profile_image
    ...
    if @user.update(picture: params[:picture])
      ...
      render status: :bad_request, json: { "message": 'profile picture updation faild' }
    end
  end
